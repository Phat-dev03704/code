...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Test MILP solver với nhiều datasets',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ví dụ sử dụng:
  # Test tất cả datasets (giới hạn 25 khách hàng/dataset)
  ...
  # Tùy chỉnh số khách hàng và thời gian
  python test_all_datasets.py --category C1 --max-customers 20 --time-limit 600

Lưu ý: MILP chỉ hiệu quả với <30 khách hàng/dataset
        """
    )
    ...
    parser.add_argument('--max-customers', type=int, default=25,
                        help='Số khách hàng tối đa (mặc định: 25)')
...
def test_with_params(categories_filter=None, limit=None, capacity=200,
                     max_vehicles=25, max_customers=25, time_limit=300,
                     save_results=True):
    ...
    print("="*100)
    print("TEST MILP VRPTW SOLVER")
    print("="*100)
    
    if categories_filter:
        print(f"Categories: {categories_filter}")
    else:
        print("Test tất cả 56 datasets")
    
    if limit:
        print(f"Giới hạn: {limit} datasets")
    
    print(f"⚠️  MILP chỉ giải {max_customers} khách hàng đầu tiên/dataset")
    ...
    if save_results and results:
        ...
        summary_file = result_dir / 'test_summary.csv'
    ...
    if success_count > 0:
        success_results = [r for r in results if r.get('Status') == 'Optimal']
